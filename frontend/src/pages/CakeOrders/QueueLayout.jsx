...
import { Clock4, LayoutGrid, CheckCircle, XCircle, ArrowRight, SmileIcon, ClipboardCheck, Calendar } from 'lucide-react'
...
const Queue = () => {
	...
	const listTabs = queueTabs.map(({ label, path, icon: Icon }, index) => <span key={index} className={`relative text-text text-sm flex items-center gap-2 cursor-pointer px-3 py-1 rounded-sm hover:bg-main-dark/50 ${path.includes(currentUrl) ? activeIndicator : (path == '' && currentUrl == 'queue') ? activeIndicator : ''}`} onClick={() => navigate(`/queue${path}`)}><Icon size={16} /> <h5>{label}</h5></span>)
